...
namespace MainForm
{
    public partial class Form2 : Form
    {
        ...
        public Form2(Service1 mockService,MyService myService)
        {
        ...
        {
            cbControlServiceLogLevel.Items.AddRange(Enum.GetNames(typeof(LogEventLevel)));
            cbWebApiLogLevel.Items.AddRange(Enum.GetNames(typeof(LogEventLevel)));
            cbMockServiceLogLevel.Items.AddRange(Enum.GetNames(typeof(LogEventLevel)));
        ...
        {
            //observer
            if (newConfig.Application == ApplicationType.MockService)
            {
                ...
                service1.savenewconfig(newConfig);                
            }

            else if (newConfig.Application == ApplicationType.ControlService) {
                LoggerHelper.Log(LogEventLevel.Information, $"ConfigChanged event triggered with new config: {newConfig.Application}, LogLevel: {newConfig.LogLevel}");
                service2.savenewconfiga(newConfig);
            }
            else if (newConfig.Application == ApplicationType.mywebapipool){
                LoggerHelper.Log(LogEventLevel.Information, $"ConfigChanged event triggered with new config: {newConfig.Application}, LogLevel: {newConfig.LogLevel}, MonitoringFrequency: {newConfig.MonitoringFrequency}");
        ...
        private LogEventLevel GetSelectedLogLevel(ComboBox comboBox)
        {
            return (LogEventLevel)Enum.Parse(typeof(LogEventLevel), comboBox.SelectedItem.ToString());
        }
